...
//        });
        var $provinceCode = $province.attr("default-province");
        var $cityCode = $city.attr("default-city");
        var $countyCode = $district.attr("default-county");
//        console.log($countyCode)
        $(areaCode.PROVINCE).each(function(i){
            if($(this)[0].code == $provinceCode){
                $province.append($("<option/>").text($(this)[0].name).attr("value",$(this)[0].code).attr("selected","selected"));
            }else{
                $province.append($("<option/>").text($(this)[0].name).attr("value",$(this)[0].code));
            }
        });
        /**/
        var data = areaCode.CITY[$provinceCode];//市
        $(data).each(function(i){
            if($(this)[0].code == $cityCode){
                $city.append($("<option/>").text($(this)[0].name).attr("value",$(this)[0].code).attr("selected","selected"));
            }else{
                $city.append($("<option/>").text($(this)[0].name).attr("value",$(this)[0].code));
            }
        });
        var data=areaCode.COUNTY[$cityCode];//县
        $(data).each(function(i){
            if($(this)[0].code == $countyCode){
                $district.append($("<option/>").text($(this)[0].name).attr("value",$(this)[0].code).attr("selected","selected"));
            }else{
