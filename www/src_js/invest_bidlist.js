...
function jsonpcallback(data){
    ...
    } else if ($('.version-table a').eq(2).hasClass('current')) {
        if(data.length < 2) return;
        $(".version-banner a").attr("href",data[1]["link"]);
        $(".version-banner img").attr("src",data[1]["picture"]);
    } else if ($('.version-table a').eq(3).hasClass('current')) {
        if(data.length < 4) return;
        $(".version-banner a").attr("href",data[3]["link"]);
        $(".version-banner img").attr("src",data[3]["picture"]);
    }
}
...
function pagedCallback(pageNo, jq) {
    ...
    var ht = $.ajax({
        ...
        success: function(ht) {
            ...
            // progressColor();
            // setProcessStyle();
            $.type(setCvsProgress) == "function" && setCvsProgress(jq.closest('div[data-type="version-part-box"]'));
